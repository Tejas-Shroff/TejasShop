...
namespace server.Service
{
    public class OrderService : IOrderService
    {
        ...
        public async Task<bool> UpdateOrderStatusAsync(int orderId, string status)
        {
            ...
            var orderDate = order.OrderDate; // Assuming OrderDate is a DateTime property in your Order entity
            var differenceInDays = (currentDate - orderDate).TotalDays;

            // Check if the order can be cancelled
            if (status == OrderStatus.Cancelled.ToString() && differenceInDays > 30)
            {
                return false; // Order cannot be cancelled if it's older than 30 days
            }
