...
export class ProductDetailComponent implements OnInit {
 ...
 ngOnInit(): void {
   ...
   this._route.paramMap.subscribe((param) => {
     ...
     if (id) {
       this.catalogService.getProductById(id).subscribe((res) => {   // Fetch the product details
         if (res.data) this.product = res.data;
       });
     }
   });
  
   this.store.select(selectCartProperty).subscribe((cart) => {  // Get the cart data
     this.cart = cart;
   });
...
//   if (currentQuantity + this.quantity > 3) {  // Validate if the total quantity exceeds 3
//     this.notification.Error('You can only add a maximum of 3 quantities per item.');
//     return;
//   }
 
//   this.store.dispatch(AddToCart({ productId, quantity: this.quantity }));  // Dispatching the action to add the product to the cart
//  }
